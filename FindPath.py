...
def animate_paths(paths):
    global canvas
    colors = ["red", "blue", "green", "yellow", "orange", "purple", "cyan", "magenta", "pink", "brown"]  # Define your colors here
    for path in paths:
...
def open_file():
    ...
    if file_path:
        ...
        try:
            ...
            result_window = tk.Toplevel(root)
            result_window.title("Erorr")
            result_label = tk.Label(result_window, text="Erorr in reading file", font=("Arial", 16))
            result_label.pack(padx=20, pady=20)
...
def main():
    ...
    canvas.pack()
    display_hospital_map(hospital_map, canvas)
    open_button = tk.Button(root, text="Open Input File", command=open_file)
    open_button.pack()
